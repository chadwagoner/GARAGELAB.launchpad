...
if [[ $nfs_backup == true ]]; then
  read -p '[REQUIRED] NFS SERVER IP ADDRESS FOR BACKUP [1.2.3.4]: ' nfs_backup_address < /dev/tty
  ...
  read -p '[REQUIRED] NFS VOLUME FOR BACKUP [volume1/directory]: ' nfs_backup_volume < /dev/tty
...
if [[ $nfs_media == true ]]; then
  read -p '[REQUIRED] NFS SERVER IP ADDRESS FOR MEDIA [1.2.3.4]: ' nfs_media_address < /dev/tty
  ...
  read -p '[REQUIRED] NFS VOLUME FOR MEDIA [volume1/directory]: ' nfs_media_volume < /dev/tty
