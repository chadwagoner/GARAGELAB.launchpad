...
mkdir -p $service_path/agents/komodo-agent

### CREATE .ENV
cat > $service_path/agents/.env <<EOF
...
### START SERVICE(S)
docker compose -f $service_path/agents/compose.yaml --env-file $service_path/agents/.env --env-file $service_path/agents/.env.komodo-agent up -d > /dev/null 2>&1
